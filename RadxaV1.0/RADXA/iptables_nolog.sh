...
#iptables -I FORWARD -p tcp -m string --string "youtube.com" --algo kmp -j DROP #esta linea bloqueaba playstore
...
#iptables -t nat -A PREROUTING -p tcp --dport http -j DNAT --to 192.168.1.250:8080
#iptables -t nat -A PREROUTING -p tcp --dport https -j DNAT --to 192.168.1.250:443
#esta regla es para que no necesiten registrarse
sudo iptables -t nat -I PREROUTING -p tcp --dport 80 -j DNAT --to-destination 192.168.1.250:3128
sudo iptables -t nat -I PREROUTING 2 -j ACCEPT
